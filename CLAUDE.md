...
> B1 sketch。**下次接續＝模式2（即時 loop，有餘裕再做：data_ready 自動觸發、CPU 不重置持續
> 跑——需另設計觸發，reset-觸發不適用 loop）；或 multi-class demo（送不同樣本得不同 class，
> 純改 PC/ESP32 送的資料，無需改硬體）。** 細節見 9.Z。
...
- **後續方向**：(1) **multi-class demo**——送不同 544-字樣本得不同 class（N/S/V/F/Q），純改
  PC/ESP32 送的資料，硬體不動（呼應 C1）。(2) **模式2 即時 loop**——data_ready 自動觸發、CPU
