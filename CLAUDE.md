...
> **★★ 最新（2026-06-19）：M2A 連續握手分類 —— ✅ 上板驗證通過 ★★**
> **✅ 上板實測通過**：切 n/s/v/f 不用 reset 即時變、ESP32 連續收 ACK 0x06、sw=11 的
> class_count 持續累加。整顆手刻 SoC 連續即時分類、零按鈕。**bringup 抓到三層 bug**：
> (1) chip_top 結果鎖存是一次性（`result_valid && !valid_lat`）→ 連續模式凍住，改成每筆
> result_valid 高就更新；(2) DMA descriptor 重用（防禦性改成每輪重設，實測非主因）；
> **(3) 真正根因＝EPU 只能單發**：頂層 global_controller FSM 會 NONE↔FINISH 重置，但內部
> 子 FSM/計數器在單發設計下不重新初始化，第二次分類算到一半 stall（`sw=11` 診斷：class_count
> 卡 1、result_valid=0、frame/dma 中斷清空＝迴圈到了 iter2 EPU 但算不完）。修法＝**每輪 EPU
> 軟重置**：EPU_Wrapper 加軟重置暫存器（子位址 `0x08` bit0 → `epu.rst_n=!ARESET & !epu_soft_reset`，
> ACLK 同步釋放），韌體每輪 DMA 後、EPU_start 前脈衝它（GLB 是 BRAM，權重+輸入留著只清控制邏輯）。
> 另加診斷：chip_top **sw=11** = led7 frame_int / led6 dma_int / led5 result_valid /
> led[3:0] class_count（迴圈健康度監看，保留）。bit=`release/chip_top_m2a.bit`（13:05 build，
> WNS +0.187ns / 0 critical）。ESP32 sketch 有一行 RX byte debug print（可選移除）。**未 commit。**
> （以下為建置當下的記錄。）
> **★ M2A 連續握手分類（建置記錄）★**
> 目標：把 M1c 的「按 reset 跑一次」改成 **boot 後自走連續辨識 + FPGA↔ESP32 握手**
> （為未來真實 ECG sensor 鋪路，使用者決策）。流程：ESP32 送 544 字幀 → FPGA loader 寫
> buffer BRAM → 升起 frame-ready 中斷 → CPU DMA buffer→GLB→啟 EPU 分類 → **EPU done 時
> 硬體送 ACK byte(0x06)** 經 uart_tx(JA7/Y21) 回 ESP32 RX(GPIO16) → ESP32 收 ACK 送下一筆。
> **strict ping-pong（等 EPU 算完才收下一筆，永久定案、不做 overlap，慢速感測器夠用）。**
> **關鍵設計（非顯而易見）**：此 CPU 只有一條 level 外部中斷（MEIP=`DMA||EPU`，靠 SW 寫
> 對應暫存器清）。raw level `data_ready` 直接 OR 會無限重觸發 → 故 frame-ready 做成
> **CPU 可清的 level latch、在 trap handler 內清**（寫 EPU reg 子位址 `0x04`=0x0006_0004）。
> ACK 用 **EPU-done 脈衝**（非 DMA-done）：真序列化＋boot 期間 EPU 不跑故不會誤觸發。
> 產出：`chip_fpga/rtl/`：`uart_tx.sv`(新,100MHz 8N1,intr_epu 上升緣送 1 byte)、
> `EPU_Wrapper.sv`(overlay：frame-ready FSM 鏡像 EPU 中斷 FSM + data_ready_cpu 輸入 +
> frame_interrupt 輸出 + 子位址 0x04 清除)、`top.sv`(data_ready 2FF 同步到 cpu_clk、
> 接 EPU_Wrapper 新埠、`interrupt` 多 OR frame_interrupt、實例化 uart_tx、uart_txd 輸出)、
> `CHIP.v`/`chip_top.sv`(穿 uart_txd 上行)。`constraints` 加 `uart_txd`=JA7/Y21 LVCMOS33。
> `build_chip.tcl` 排除原 EPU_Wrapper、收 overlay+uart_tx。Firmware `main.c`=無窮迴圈
> `wfi(frame)→DMA→wfi(dma)→EPU_start→wfi(epu)`，handler 多清 FRAME_CLEAR(0x0006_0004)；
> boot.c 不變（M1c 版，不載 buffer，buffer 由 UART 餵，fallback 全 0）。rom*.hex 已重編
> 並複製進 chip_fpga/。ESP32：`ESP32_Arduino_UART/esp32_epu_m2a/`（ACK 驅動連續送、重複某
> 一 class n/s/v/f、800ms timeout 重送自我同步 bootstrap/丟包）。**HW(chip_top)未來真實
> 感測那步不需改。** Vivado build 進行中（synth 已過、impl 中），bit 將為
> `chip_fpga/build/chip_top.bit`→release `chip_top_m2a.bit`。**接線**：使用者已接 JA7(TX)
> ↔GPIO16(RX)。**下次接續＝上板驗證**：USB 燒 m2a bit + ESP32 跑 sketch → 應自走連續分類、
> sw=01 顯示對應 class、無需按鈕。驗過後 commit。細節見第 9 節 9.Z 末。
> （另：已設 Stop hook 完成音效通知 `notify.wav`，在使用者全域 settings.json。）
...
> （鐵證）。細節見第 9 節 9.Z。
> **✅✅ 模式1 M1c 上板驗證通過（2026-06-15）—— 完整模式1 達成**：UART RX 寫 buffer
> 第二埠（true→改 simple dual-port BRAM：port A 純讀/port B 純寫）。實測：無 UART→sw=00
> =0x6B(S，零輸入 fallback)；ESP32 送 golden 幀→sw=10 rx_seen/data_ready 亮、uart_err 暗→
> 按 BTNC reset→sw=00=0x67(N)。結果隨 UART 改變＝鐵證，整條 UART→buffer BRAM→CPU/DMA→GLB
> →EPU 打通，資料在 BRAM、reset 不清。bit=`chip_fpga/release/chip_top_m1c.bit`。ESP32 沿用
> B1 sketch。
> **✅ multi-class demo 已建好（2026-06-15，待上板）**：逆向+驗證 GLB tiling layout（ECG 3 筆/
> word @ stride4、零點 pad 0x31；RR@word512、pad 0x7F），管線 extract→`pack_glb_input.py`→
> `gen_multiclass_header.py`→ESP32 `samples.h`。4 類樣本（N/S/V/F，Q 略過）在
> `chip_fpga/demo_samples/`，ESP32 sketch 在 `ESP32_Arduino_UART/esp32_epu_multiclass/`。硬體不動
> （同 chip_top_m1c.bit）。**✅ 上板驗證通過（2026-06-15）**：n/s/v/f→BTNC→sw=01 class index
> N=000/S=001/V=010/F=011 全對，整顆手刻 SoC 從真實 MIT-BIH ECG 串流分出 4 類心律。
> **★ 專案暫停／關閉（2026-06-15，使用者收工）★** 目前＝**可交付版本**（ESP32 送 4 type →
> FPGA LED 驗證），多類別已硬體驗證。交付資料夾整理在 **`deliverable/`**（fpga bit+源碼+
> 韌體+tools+demo_samples、esp32 sketch、頂層 README）——**此資料夾未 commit（使用者要求保留本機、
> 不進 git）**；GitHub `main` 已含全部里程碑源碼與能跑的 bit（`chip_fpga/release/`）。
> **環境（下次可直接用）**：WSL2 Ubuntu-24.04 已裝 RISC-V 工具鏈（`riscv64-unknown-elf-`）+
> venv `~/aoc_venv`（numpy/scipy/wfdb/sklearn/torch-cpu，產樣本用）。
> **★ 下次接續＝真實感測前端 ★**：ESP32 接 ECG sensor，**16-bit raw → 韌體端 normalize 到
> int8 → tiling（見 9.Z 的 GLB layout）→ 經 UART 送 buffer**（使用者偏好韌體端處理）。起點：
> `ESP32_Arduino_UART/esp32_ble_ecg/esp32_ble_ecg.ino`（BLE ECG，已存在）。PC 端前處理/量化參數
> （ecg/rr scale+zp）在 `software/train/*.npz`。硬體（chip_top_m1c）不需改，前端只要把即時資料
> 做成正確的 544-字 tiled 影像送進來、按 reset 即可。
> 其他可選：模式2 即時 loop（需另設計觸發、reset 不適用 loop）。
> 進 EPU/位址/tiling 細節看第 9 節（9.X 資料路徑、9.Y B0、9.Z M1a~c+multi-class）。
> 以下里程碑 1 與轉向背景為歷史記錄。
...
**✅ M1c 完成（2026-06-15）：完整模式1 上板驗證通過。**
- 產出（chip_fpga/rtl/）：`uart_rx.sv`（搬自 epu_fpga，參數 CLK_HZ=50M）、`uart_buffer_loader.sv`
  （新；uart_rx + 框架 FSM AA55+2176B+XOR → 寫 buffer port B，無 start 按鈕）、
  `SRAM_wrapper_buf.sv`（加 port B；**注意：原本想做 byte-write port A + 全字 port B 的 TDP，
  Vivado 報 `Unsupported RAM template`——byte-write 與全字混用無法推斷。改成 simple dual-port：
  port A 純讀、port B 純寫。M1c 中 AXI 從不寫 buffer，故 port A 去掉寫邏輯安全**）、
  `AXI_BRAM_Buffer_wrapper.sv`（實例化 loader + 接 port B + 拉 uart_rxd/status）、
  `CHIP.v`+`top.sv` overlay（穿 uart_rxd 下行 + data_ready/uart_err/rx_seen 上行）、
  `chip_top.sv`（uart_rxd pin、status 2FF 同步到 100MHz、sw=10 顯示 UART 狀態）。
  `constraints`：uart_rxd=JA1/AB22 LVCMOS33。`build_chip.tcl`：多排除原 top.sv/CHIP.v、多收
  uart_rx/uart_buffer_loader/top/CHIP overlay。**fallback 改零**（buf_init.hex 全 0；golden 留
  `buf_init_golden_m1b.hex`），讓「無 UART→S、UART→N」成對照鐵證。韌體不變（同 M1b）。
- build：WNS +0.063ns、0 失敗、0 critical，buf_init.hex/rom 載入，UART 模組合成。
  bit：`build/chip_top_m1c_uart.bit`、release `chip_top_m1c.bit`。
- ✅ 上板（USB + ESP32）：無 UART→sw=00=0x6B(S)；ESP32 送 golden→sw=10 rx_seen/data_ready
  亮+uart_err 暗→按 BTNC→sw=00=0x67(N)。**0x6B→0x67 隨 UART 改變＝完整模式1 鐵證。**
  資料在 BRAM、reset 不清（符合使用者約束）。順序：送 UART→等 data_ready→按 BTNC。
- **✅ multi-class demo 已建好（2026-06-15，硬體不動，純改送的資料）**：
- **GLB 輸入 tiling layout（逆向+對 golden 逐位元驗證+組員確認）**：輸入是「單筆樣本以 tile
  為單位 padding，padding 值＝該輸入的零點」。**ECG**：198 筆做成 66 row×3，每 row 放在
  GLB word `4*r`（stride 4），byte0-2＝ECG[3r..3r+2]、byte3＝ECG 零點 **0x31**；該 row 後 3 個
  word 全 `0x31313131`；共 72 row×4＝words 0..287（row 66-71 無資料填 0x31313131），words
  288..511 補 0。**RR**：BASE 在 word **512**，word512＝`rr0|rr1<<8|0x7F<<16|0x7F<<24`
  （RR 零點 **0x7F**），words 513..543＝`0x7F7F7F7F`。decoder.sv `BASE_ECG_IN=0`/`BASE_RR_IN=512`。
  word 545~1024 不重要（補 0 即可），1024+ 是權重區。
- **產生管線**：`extract_mitdb_one_case_layer_io.py --label N/S/V/F --label-occurrence 0
  --split test_clean`（出 198 ECG+2 RR dense packet，需 WSL venv `~/aoc_venv`：numpy/scipy/
  wfdb/sklearn/torch-cpu）→ `chip_fpga/tools/pack_glb_input.py`（dense→544 tiled，已對 golden
  逐位元驗證）→ `chip_fpga/tools/gen_multiclass_header.py`（→ ESP32 `samples.h`）。
- **預期 class（bittrue golden model argmax 確認）**：N=[idx0,0x67]、S=[idx1,0x6B]、
  V=[idx2,0x65]、F=[idx3,0x8A]。**Q 略過**（訓練樣本太少，模型分不出，occ0 被分成 V）。
- **ESP32**：`ESP32_Arduino_UART/esp32_epu_multiclass/`（`.ino`+`samples.h`+README）。Serial 打
  n/s/v/f 選送 → 等 data_ready → 按 BTNC reset → FPGA sw=01 顯示 class index（N=000/S=001/
  V=010/F=011）、sw=00 顯示 score。樣本 .dat 在 `chip_fpga/demo_samples/in_{N,S,V,F}.dat`。
- **✅ 上板驗證通過（2026-06-15）**：Serial 送 n/s/v/f → 按 BTNC → sw=01 class index 依序
  N=000、S=001、V=010、F=011，4 類全對。整顆手刻 SoC 從真實 MIT-BIH ECG（UART 串流）分出
  4 種心律類別（正常/室上性/室性/融合波），完整即時多類別分類系統打通。

**模式2 即時 loop（更後續）**——data_ready 自動觸發、CPU
  持續跑不重置：reset-觸發不適用（reset 會清 CPU），需回到「CPU 不重置 + 觸發機制」，觸發要
  避開 cache（中斷或 EPU reg 區的非 cacheable 旗標），是較大工程，有餘裕再做。
