...
> `chip_fpga/firmware/`。**下次接續＝M1b**（加按鈕觸發：CPU 等 BTNC 才搬資料，需新增
> CPU 可讀的狀態/觸發 slave 或中斷）→ 之後 M1c（UART 填 buffer 取代 boot 預載）。
> 細節見第 9 節 9.X/9.Y/9.Z。
...
**M1b（下一步）**：把 main.c 的「無條件 DMA」改成「等觸發才 DMA」——需要 CPU 讀得到按鈕
（BTNC）或 data_ready。兩條路：(a) 新增極小狀態暫存器 slave 供 CPU 輪詢；(b) 把按鈕 OR 進
中斷線(`DMA_interrupt||intr_epu`)。M1a 證明資料路已通，M1b 只加觸發控制。
**M1c**：buffer 開第二埠（true dual-port），UART RX（搬 B1 的 uart_rx.sv + 框架解析）寫入，
取代 boot 的 ROM→buffer 預載 → 真正串流任意輸入。
